/**
 * TRPG 후속탐구 기록 백엔드 (후속탐구 · 1차 계획 조회 API 포함)
 * ────────────────────────────────────────────────────
 * 1. 학생은 HTML 워크북에서 작성 → POST 제출 (학년·반·번호·이름 직접 입력)
 * 2. 'TRPG_후속' 시트에 upsert (학년-반-번호-이름 키, 빈 값은 기존 값 보존 = 병합 저장)
...
// ─── 상수 ───
const SHEET_NAME = 'TRPG_후속';
const PLAN_SRC = { id: '1_I8RSq-Y_jqsJ9R_8XpWi-PiUCVatgTBmxDMuKwUy2o', tab: 'BTS_제출', col: '후속연구' };
...
function doGet(e) {
  ...
  return tpl.evaluate()
    .setTitle('Beyond the Screen 후속탐구 — 학급 보드')
...
function buildStudentTab(student, rec) {
  ...
  content[0] = [student.name + '(' + student.grade + '-' + student.ban + '-' + student.num + ') — TRPG 후속탐구 기록', ''];
...
function ensureGuideTab() {
  ...
  const lines = [
    ...
    ['  2. 시트 → 자동으로 [TRPG_후속] 탭에 row 추가/갱신 (병합 저장 — 빈 칸은 기존 값 보존)', ''],
...
function generateSetukDraft(rec) {
  ...
  const parts = [];
  if (title) {
    parts.push("영어·일반사회 융합수업(TRPG)에서 예고한 탐구를 '" + clip(title, 70) + "' 후속탐구로 실행함.");
  }
...
function exportCurrentToDoc() {
  ...
  const title = 'TRPG후속_' + name + '_' + Utilities.formatDate(new Date(), 'GMT+9', 'yyyyMMdd');
  const doc = DocumentApp.create(title);
  const body = doc.getBody();
  const H1 = DocumentApp.ParagraphHeading.HEADING1;
  const H2 = DocumentApp.ParagraphHeading.HEADING2;

  body.appendParagraph('TRPG 후속탐구 기록 — ' + name).setHeading(H1);
...
function resetAllSubmissions() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const ui = SpreadsheetApp.getUi();
  const r = ui.alert('⚠ [TRPG_후속] 시트의 모든 학생 행을 지웁니다. 학생 카드 탭은 유지됩니다. 정말?', ui.ButtonSet.YES_NO);
  if (r !== ui.Button.YES) return;
